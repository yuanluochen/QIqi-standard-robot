...
void chassis_task(void const *pvParameters)
{
    ...
    while (1)
    {
        ...
        {
            //发送控制数据
            CAN_cmd_chassis(chassis_move.chassis_relative_angle, chassis_move.vx_set, chassis_move.vy_set, chassis_move.chassis_behaviour);
        }
...
static void chassis_feedback_update(chassis_move_t *chassis_move_feedback_update)
{
    ...
    chassis_move_feedback_update->chassis_relative_angle = chassis_move_feedback_update->chassis_yaw_motor->gimbal_motor_measure->ecd - chassis_move_feedback_update->chassis_yaw_motor->frist_ecd;
    if (chassis_move_feedback_update->chassis_relative_angle > 4096)
        chassis_move_feedback_update->chassis_relative_angle -= 8191;
    else if (chassis_move_feedback_update->chassis_relative_angle < -4096)
        chassis_move_feedback_update->chassis_relative_angle += 8191;
}
